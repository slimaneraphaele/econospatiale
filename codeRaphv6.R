...
#setwd("/shares/espert/w3crk9/2018-05-13-econospatialeRaph")
...
# on joint les donn?es locales avec les donnees concernant les EPCI
...
city_all <- city_all %>%
  dplyr::rename(INSEE_COM = `cog (code officiel g?ographique)`,
                ...
                taxe_habit_base = `taxe d'habitation (y compris thlv) - base nette impos?e`,
                ...
                taxe_fonciere_non_bati_base =`taxe fonci?re sur les propri?t?s non b?ties - base nette impos?e`,
                ...
                taxe_fonciere_bati_base = `taxe fonci?re sur les propri?t?s b?ties - base nette impos?e`,
...
xtable(results_moran, caption = "Matrice contiguit? Wq")
#xtable(results_moran, caption = "Matrice contiguit? Wr")
#xtable(results_moran, caption = "Matrice contiguit? Wdist")
...
colnames(df) <- c("TH stat", "TH pval", "TFNB stat", "TFNB pval","TFB stat",
                  "TFNB pval")
xtable(df, digits = 2, caption = "R?sultats des tests de Lagrange")
# il y a de l'autocorr?lation
...
# Fit SAR model -----------------------------------------
# petite modif des mod?les pour la variable base
...
# Mise en forme du tableau de r?sultats
...
print(xtable(as.is(results), caption = "Mod?le SAR", digits = 3, auto = TRUE,
...
load("results_sar.RData")
W <- results_sar[["W"]]
fit_sar_th <- results_sar[["fit_sar_th"]]
fit_sar_tfb <- results_sar[["fit_sar_tfb"]]
fit_sar_tfnb <- results_sar[["fit_sar_tfnb"]]

calculate_impact_sar_stsls <- function(obj, listw){
  ...
  Vsp <- as(V, "dgCMatrix")
  rm(V)
  # on prend la plus petite et la plus grande valeur propre (plus gde partie rÃ©elle)
  emax <- RSpectra::eigs(Vsp, k, which = "LR", opts = list(retvec =F))$values
  emin <- RSpectra::eigs(Vsp, k, which = "SR", opts = list(retvec =F))$values
  e <- c(emax, emin)
  if (is.complex(e)) {
    interval <- 1/(range(Re(e)))
  }else {
    interval <- 1/(range(e))}
  if (rho <= interval[1] || rho >= interval[2]){
  ...
  df <- data.frame("direct" = direct, "indirect" = indirect,
                   "total" = total)
  row.names(df) <- bnames
  results <- df
  return(results)
}
impacts_th <- calculate_impact_sar_stsls(fit_sar_th,W)
impacts_tfb <- calculate_impact_sar_stsls(fit_sar_tfb,W)
impacts_tfnb <- calculate_impact_sar_stsls(fit_sar_tfnb,W)
impacts <- list("impacts_th" = impacts_th, "impacts_tfb" = impacts_tfb,
                "impacts_tfnb"=impacts_tfnb)
save(impacts, file= "impacts.RData")
load("impacts.RData")
row_names <- c("Base fiscale nette (log)", "Score urbain", "Nb. logements (log)",
               "Population (log)", "Nb. ménages (log)", "Evolution pop",
               "Moy. revenus fiscaux (log)", "Moy. revenus fiscaux dép. (log)",
               "Encours dette (log)", "Dotation fonctionnelle (log)",
               "Type fisca. : FA", "Type fisca. : FPU", 
               "Nb. communes EPCI", "Services publics (log)",
               "Indica. région 28", "Indica. région 32", 
               "Indica. région 44")
df <- impacts[[1]]
row.names(df) <- row_names
xtable(df,digits = 4, caption ="TH")
df <- impacts[[2]]
row.names(df) <- row_names
xtable(df,digits = 4, caption ="TPFB")
df <- impacts[[3]]
row.names(df) <- row_names
xtable(df,digits = 4, caption = "TPFNB")
...
comment$command <- c(paste("\\hline\n",
                           "{\\footnotesize \\textit{Note:}   $^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01}\n", sep = ""))
print(xtable(as.is(results), caption = "Mod?le SEM", digits = 3, auto = TRUE,
             align= c("l","c","c","c","c","c","c")), add.to.row = comment,
      hline.after = hlines,
      sanitize.text.function = identity,booktabs = T,size="\\fontsize{9pt}{10pt}\\selectfont")
...
comment$command <- c(paste("\\hline\n",
                           "{\\footnotesize \\textit{Note:}   $^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01}\n", sep = ""))
print(xtable(as.is(results), caption = "Mod?le SARAR", digits = 3, auto = TRUE,
             align= c("l","c","c","c","c","c","c")), add.to.row = comment,
      hline.after = hlines,
      sanitize.text.function = identity,booktabs = T,size="\\fontsize{9pt}{10pt}\\selectfont")
...
calculate_impact_sarar_gmsar <- function(obj,listw){
...
impacts_th <- calculate_impact_sarar_gmsar(fit_sarar_th,W)
impacts_tfb <- calculate_impact_sarar_gmsar(fit_sarar_tfb,W)
impacts_tfnb <- calculate_impact_sarar_gmsar(fit_sarar_tfnb,W)
impacts_sarar <- list("impacts_th" = impacts_th, "impacts_tfb" = impacts_tfb,
                      "impacts_tfnb"=impacts_tfnb)
save(impacts_sarar, file= "impacts_sarar.RData")
load("impacts_sarar.RData")
...
# ----------------------------------------------------
load("results_sarar_coupe.RData")
W <- results_sar[["W"]]
fit_sarar_th <- results_sarar[["sarar_th"]]
fit_sarar_tfb <- results_sarar[["sarar_tfb"]]
fit_sarar_tfnb <- results_sarar[["sarar_tfnb"]]
...
T_
tab2 <- tab2 %>% dplyr::filter(annee !=1) # on restreint ? 3 ans (pb de trop gde dim sinon)
...
# Test d'Hausman robuste ? l'autocorr?lation spatiale (splm)
...
imp_th <- impacts(sarar_fe_spatial_th, listw=W, time = 3) # 3 p?riodes
